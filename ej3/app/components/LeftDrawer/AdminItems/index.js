...
class AdminItems extends React.Component {
  constructor(props) {
    ...
    this.handleAdminItemsNestedListToggle = this.handleAdminItemsNestedListToggle.bind(this);
  ...
  componentWillReceiveProps(newProps) {
    ...
    if ((newProps.appStore.adminMenus.length > 0
      && newProps.appStore.adminMenus.length === this.props.appStore.adminMenus.length && !this.menuLoaded)
  ...
  handleAdminItemsNestedListToggle = (item) => {
    if (!this.state.isMobileBrowser) {
      if (item.props.open) {
  ...
  render() {
    ...
    console.log("this.props.appStore.adminMenus", this.props.appStore.adminMenus)
    return (
      <SelectableList
      ...
      >
        {
          this.props.appStore.adminMenus.length > 0 ? (
            <ListItem
              value={-1}
              ...
              nestedItems={this.props.appStore.adminMenus.length > 0 ?
                this.props.appStore.adminMenus.map((menu) =>
                  <ListItem
                    ref={(listItem) => {
                    ...
                    onNestedListToggle={this.handleAdminItemsNestedListToggle}
                    open={menu.open}
                    data-id={menu.id}
                    data-url={menu.url}
